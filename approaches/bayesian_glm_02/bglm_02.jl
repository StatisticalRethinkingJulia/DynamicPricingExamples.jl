...
price_points = [12.5, 12.55, 13.02, 14.20, 15.15] # Selected price points
...
if success(rc)
	...
	price_points_l = log.(price_points)
	scatter(price_points_l, a_mean .+ b_mean .* price_points_l,
		xlims = (2.4, 2.75), ylims= (6.1, 6.5), leg=false,
		xlabel="log(ppu)", ylabel="log(quantity sold)")
	selected_rows = rand(1:size(dfs, 1), 50)
